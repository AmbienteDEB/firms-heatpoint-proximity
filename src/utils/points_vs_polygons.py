...
def read_points(csv_path: Path, crs: str) -> gpd.GeoDataFrame:
...
def run_spatial_analysis(cfg: Config, csv_hotspots_path:Path, csv_computed_output_dir:Path):
    points = read_points(csv_hotspots_path, cfg.points_crs)
    polygons = read_polygons(cfg.polygons_path, cfg.polygons_name_field, cfg.forced_polygons_crs)
    ...
    csv_computed_output = csv_computed_output_dir / "computed.csv"

    output.to_csv(csv_computed_output, index=False, encoding="utf-8")
